...
class memoized(object):
    ...
    def __get__(self, obj, objtype):
        ...
        return fn
    def _reset(self, item=None):
        # TODO remove individual item from stash
        if item is None:
            print "[memoize] reset cache"
            self.cache = {}
        else:
            print "[memoize] remove %s from cache" % str(item) 
