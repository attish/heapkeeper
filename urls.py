...
urlpatterns = patterns('hk.views',
    ...
    url(r'^replymessage/(?P<obj_id>\d+)/$',
        view='replymessage',
        name='replymessage'),
    url(r'^wait_for_conv_update/(?P<conv_id>\d+)/\d+/$',
            # the second numeric parameter is a cache killer
        view='wait_for_conv_update',
        name='wait_for_conv_update'),
    url(r'^touch_conv/(?P<conv_id>\d+)/$',
        view='touch_conv',
        name='touch_conv'),
